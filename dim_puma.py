...
class TransformStep(PipelineStep):
    def run_step(self, prev, params):  
        df = pd.read_csv(prev)  
        df=df[['puma_id','puma_name']] #I select the variable I've needed
...
class ExamplePipeline(EasyPipeline):
    ...
    def steps(params):
        ...
        dtype={'puma_id':'String',
               'puma':'String'}
        load_step=LoadStep('dim_puma',db_conector,if_exists='drop',dtype=dtype,pk=['puma_id'],nullable_list=[]) #borrar drop, append agregar datos
